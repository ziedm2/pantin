...
# Install agency backend dependencies
echo "
################################################
#                INSTALL AGENCY BACKEND               #
################################################
"
cd /home/agency/backend || exit
npm install --f
cd /home/agency/backend || exit
...
################################################
"
cd /home/agency/frontend/src || exit
cat > key.js << EOF
export const authKey = "$shared_secret_key";
EOF

cd /home/agency/frontend/src/util || exit
cat > config.js << EOF
export const baseURL = "http://$public_ip:5000/";
export const projectName = "$app_name";
EOF
...
nvm install node
npm run build
sudo mv /home/agency/frontend/build/* /home/agency/backend/public
...
server {
    ...
    access_log /var/log/nginx/example.access.log;  #whatever your server name
    error_log /var/log/nginx/example.error.log;   #whatever your server name
    root /path;
    location / {
...
3. agencyPanel : http://$public_ip:5001/
